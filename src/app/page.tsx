...
export default function Home() {
  ...
  function projectRows() { return <div className={styles.projectList}>{filteredProjects.length ? filteredProjects.map((project) => { const visual = projectCatalog.find((item) => item.id === project.project_id) ?? { color: "amber", icon: Settings2 }; const Icon = visual.icon; return <article className={styles.project} key={project.project_id}><div className={`${styles.projectIcon} ${styles[visual.color]}`}><Icon size={18} /></div><div className={styles.projectInfo}><strong>{project.name}</strong><small>{project.description ?? "Módulo Binc"}</small><p>{project.capabilities?.join(" · ") ?? "Sem capacidades informadas"}</p></div><span className={`${styles.status} ${styles[visual.color]}`}><i /> {statusLabel(project.status)}</span><Link href={`/projects/${project.project_id}`} className={styles.textButton}>Abrir <ArrowUpRight size={13} /></Link></article>; }) : <p className={styles.emptyState}>Nenhum projeto encontrado.</p>}</div>; }
